...
CleanroomLogger can write messages to Apple's new Unified Logging System, to the UNIX `stdout` and `stderr` streams, and to files. CleanroomLogger is also heavily extensible, so you can roll your own implementations that send messages over sockets or HTTP connections.
