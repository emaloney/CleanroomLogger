...
**Important:** This is the `live-log-viewer` branch. It uses **Swift 3.0.2** and **requires Xcode 8.2** to compile.
...
[`live-log-viewer`](https://github.com/emaloney/CleanroomLogger/tree/live-log-viewer)|[![Build status: live-log-viewer branch](https://travis-ci.org/emaloney/CleanroomLogger.svg?branch=live-log-viewer)](https://travis-ci.org/emaloney/CleanroomLogger)
...
- [CleanroomLogger In Depth](#cleanroomlogger-in-depth)
	...
	- [API Documentation](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/index.html)
...
Good documentation is critical to the usefulness of any open-source framework. In addition to the extensive high-level documentation you’ll find below, [the CleanroomLogger API](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/index.html) itself is 100% documented.

#### ▶ Organize and filter messages by severity

Messages are assigned one of five [_severity levels_](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Enums/LogSeverity.html): the most severe is _error_, followed by _warning_, _info_, _debug_ and _verbose_, the least severe. Knowing a message’s severity lets you perform additional filtering; for example, to minimize the overhead of logging in App Store binaries, you could choose to log only warnings and errors in release builds.
...
Support for standard UNIX output streams is built-in. Use [`StandardOutputLogRecorder`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/StandardOutputLogRecorder.html) and [`StandardErrorLogRecorder`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/StandardErrorLogRecorder.html) to direct output to `stdout` and `stderr`, respectively.

Or, use the [`StandardStreamsLogRecorder`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/StandardStreamsLogRecorder.html) to send verbose, debug and info messages to `stdout` while warnings and errors go to `stderr`.
...
CleanroomLogger provides [simple file-based logging](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/FileLogRecorder.html) support as well as [a self-pruning rotating log directory](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/RotatingLogFileConfiguration.html) implementation.
...
CleanroomLogger ships with two general-purpose log formatters: the [`ReadableLogFormatter`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/ReadableLogFormatter.html) is handy for human consumption, while the [`ParsableLogFormatter`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/ParsableLogFormatter.html) is useful for machine processing. Both can be customized via the initializer.
...
If the built-in formatters don’t fit the bill, you can use the [`FieldBasedLogFormatter`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Classes/FieldBasedLogFormatter.html) to assemble just about any kind of log format possible.
...
CleanroomLogger exposes three primary extension points for implementing your own custom logic:
  - [`LogRecorder`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Protocols/LogRecorder.html)s are used to record formatted log messages. Typically, this involves writing the message to a stream or data store of some kind. You can provide your own `LogRecorder` implementations to utilize facilities not natively supported by CleanroomLogger: to store messages in a database table or send them to a remote HTTP endpoint, for example.
  - [`LogFormatter`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Protocols/LogFormatter.html)s are used to generate text representations of each [`LogEntry`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Structs/LogEntry.html) to be recorded.
  - [`LogFilter`](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/Protocols/LogFilter.html)s get a chance to inspect—and potentially reject—a `LogEntry` before it is passed to a `LogRecorder`.


### License

CleanroomLogger is distributed under [the MIT license](https://github.com/emaloney/CleanroomLogger/blob/live-log-viewer/LICENSE).
...
Finally, you’ll need to [integrate CleanroomLogger into your project](https://github.com/emaloney/CleanroomLogger/blob/live-log-viewer/INTEGRATION.md) in order to use [the API](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/index.html) it provides.
...
See [the Integration document](https://github.com/emaloney/CleanroomLogger/blob/live-log-viewer/INTEGRATION.md) for additional details on integrating CleanroomLogger into your project.
...
For detailed information on using CleanroomLogger, [API documentation](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/index.html) is available.
...
[API documentation for CleanroomLogger](https://rawgit.com/emaloney/CleanroomLogger/live-log-viewer/Documentation/API/index.html) is generated using [Realm](http://realm.io)’s [jazzy](https://github.com/realm/jazzy/) project, maintained by [JP Simard](https://github.com/jpsim) and [Samuel E. Giddins](https://github.com/segiddins).
