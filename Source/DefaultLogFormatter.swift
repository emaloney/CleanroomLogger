...
public struct DefaultLogFormatter: LogFormatter
{
    ...
    /** The color table used to colorize log entries. If `nil`, no colors are applied. */
    public let colors: ColorTable?

    /** When `colors` is non-`nil`, the value of this property determines whether
    the full log message is colorized, or just the metadata prefix.  */
    ...
    :param:     colors The color table used to apply color formatting. If `nil`,
                no colorization is applied.

    :param:     colorizePrefixOnly If `true` and `colors` is non-`nil`, colors
                will be applied only to the metadata prefix of the log message
                as opposed colorizing the entire prefix.
    */
    public init(includeTimestamp: Bool = false, includeThreadID: Bool = false, colors: ColorTable? = DefaultColorTable(), colorizePrefixOnly: Bool = false)
    {
    ...
    */
        ...
        -> String?
    {
        ...
        return DefaultLogFormatter.formatLogMessageWithSeverity(entry.severity, caller: caller, message: message, timestamp: timestamp, threadID: threadID, colors: colors, colorizePrefixOnly: colorizePrefixOnly)
    }
    ...
    :param:     severity The log entry's severity.
    ...
    */
    public static func formatLogMessageWithSeverity(severity: LogSeverity, caller: String, message: String, timestamp: String?, threadID: String?, colors: ColorTable?, colorizePrefixOnly: Bool)
        -> String
    {
        ...
        if colors != nil && colorizePrefixOnly {
            fmt = colors!.colorizeString(fmt, forSeverity: severity)
        }
